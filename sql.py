...
"""
    ...
    def get_applicant_meetings(self, user_id):
        ...
        status IN ('Подтверждена', 'Перенесена', 'Отклонена соискателем (перенос)', 'Инициирована экспертом (перенос)',
        'Инициирована соискателем (перенос)', 'Отклонена экспертом (перенос)')
        '''
        return self.execute(sql, fetchall=True)
