...
async def check_profile_a(call: CallbackQuery):
    ...
    track_user_activity(user_id, "applicants", "изменения профиля")
    ...
    logger.debug(f'Applicant {user_id} entered check_profile_a handler')
...
async def warn_change_role_a(call: CallbackQuery):
    ...
    track_user_activity(user_id, "applicants", "Сменить роль")
...
""",
    ...
    await call.message.edit_reply_markup()
    logger.debug(f'Applicant {user_id} entered warn_change_role_a handler')
...
async def change_profile_a(call: CallbackQuery):
    ...
    track_user_activity(user_id, "applicants", "Хочу изменить")

    await call.message.answer(text="Какое поле ты хочешь изменить?",
                              reply_markup=applicant_profile_bk,
                              disable_notification=True)
    await call.message.edit_reply_markup()
    logger.debug(f'Applicant {user_id} entered change_profile_a handler')
...
async def cha_firstname(call: CallbackQuery, state: FSMContext):
    ...
    track_user_activity(user_id, "applicants", "Имя")

    await call.message.answer(text="Напиши свое имя",
                              disable_notification=True)
    await state.set_state('cha_firstname')
    await call.message.edit_reply_markup()
    logger.debug(f'Applicant {user_id} entered cha_firstname handler')
...
async def cha_firstname2(message: Message, state: FSMContext):
    ...
    await message.answer(text=f"Твое имя изменено на <i>{message.text}</i>",
                         reply_markup=applicant_profile_bk,
                              disable_notification=True)
    await state.finish()
    logger.debug(f'Applicant {user_id} entered cha_firstname2 handler')
...
async def cha_lastname(call: CallbackQuery, state: FSMContext):
    ...
    track_user_activity(user_id, "applicants", "Фамилия")

    await call.message.answer(text="Напиши свою фамилию",
                              disable_notification=True)
    await state.set_state('cha_lastname')
    await call.message.edit_reply_markup()
    logger.debug(f'Applicant {user_id} entered cha_lastname handler')
...
async def cha_lastname2(message: Message, state: FSMContext):
    ...
    await message.answer(text=f"Твоя фамилия изменена на <i>{message.text}</i>",
                         reply_markup=applicant_profile_bk,
                              disable_notification=True)
    await state.finish()
    logger.debug(f'Applicant {user_id} entered cha_lastname2 handler')
...
async def cha_direction(call: CallbackQuery, state: FSMContext):
    ...
    track_user_activity(user_id, "applicants", "направление")
    ...
    logger.debug(f'Applicant {user_id} entered cha_direction handler')
...
async def cha_direction2(call: CallbackQuery, state: FSMContext):
    ...
    track_user_activity(user_id, "applicants", "выбора направления")
    ...
    await call.message.edit_reply_markup()
    await state.finish()
    logger.debug(f'Applicant {user_id} entered cha_direction2 handler')
...
async def cha_profile(call: CallbackQuery, state: FSMContext):
    ...
    track_user_activity(user_id, "applicants", "Опыт")
    ...
    await call.message.edit_reply_markup()
    logger.debug(f'Applicant {user_id} entered cha_profile handler')
...
async def cha_profile2(message: Message, state: FSMContext):
    ...
    await message.answer(text=f"Твой опыт был изменен",
                         reply_markup=applicant_profile_bk,
                              disable_notification=True)
    await state.finish()
    logger.debug(f'Applicant {user_id} entered cha_profile2 handler')
...
async def cha_institution(call: CallbackQuery, state: FSMContext):
    ...
    track_user_activity(user_id, "applicants", "Учебное заведение")
    ...
    await call.message.edit_reply_markup()
    logger.debug(f'Applicant {user_id} entered cha_institution handler')
...
async def cha_institution2(message: Message, state: FSMContext):
    ...
    await message.answer(text=f"Твой учебное заведение было изменено",
                         reply_markup=applicant_profile_bk,
                              disable_notification=True)
    await state.finish()
    logger.debug(f'Applicant {user_id} entered cha_institution2 handler')
...
async def cha_grad_year(call: CallbackQuery, state: FSMContext):
    ...
    track_user_activity(user_id, "applicants", "Год окончания")
    ...
    await call.message.edit_reply_markup()
    logger.debug(f'Applicant {user_id} entered cha_grad_year handler')
...
async def cha_grad_year2(message: Message, state: FSMContext):
    ...
    await message.answer(text=f"Твой год окончания учебного заведения был изменен",
                         reply_markup=applicant_profile_bk,
                              disable_notification=True)
    await state.finish()
    logger.debug(f'Applicant {user_id} entered cha_grad_year2 handler')
...
async def cha_empl_region(call: CallbackQuery, state: FSMContext):
    ...
    track_user_activity(user_id, "applicants", "Регион трудоустройства")
    ...
    await call.message.edit_reply_markup()
    logger.debug(f'Applicant {user_id} entered cha_empl_region handler')
...
async def cha_empl_region2(message: Message, state: FSMContext):
    ...
    await message.answer(text=f"Твой желаемый регион для трудоустройства был изменен",
                         reply_markup=applicant_profile_bk,
                              disable_notification=True)
    await state.finish()
    logger.debug(f'Applicant {user_id} entered cha_empl_region2 handler')
...
async def cha_hobby(call: CallbackQuery, state: FSMContext):
    ...
    track_user_activity(user_id, "applicants", "Хобби")
    ...
    await call.message.edit_reply_markup()
    logger.debug(f'Applicant {user_id} entered cha_hobby handler')
...
async def cha_hobby2(message: Message, state: FSMContext):
    ...
    await message.answer(text=f"Твои хобби были изменены",
                         reply_markup=applicant_profile_bk,
                              disable_notification=True)
    await state.finish()
    logger.debug(f'Applicant {user_id} entered cha_hobby2 handler')
...
async def cha_topics(call: CallbackQuery, state: FSMContext):
    ...
    track_user_activity(user_id, "applicants", "Темы на обсуждение")
    ...
    logger.debug(f'Applicant {user_id} entered cha_topics handler')
...
async def cha_topics2(call: CallbackQuery, state: FSMContext):
    ...
    track_user_activity(user_id, "applicants", "выбора темы на обсуждение")
    ...
    if cdata != 'done':  # if user dont press "Done" button
        ...
        if sdata.get('list') is None or not sdata['list']:  # if user did not choose any button
            ...
            await call.message.edit_reply_markup()
            await state.finish()
    logger.debug(f'Applicant {user_id} entered cha_topics2 handler with cd {call.data}')
...
async def cha_topics_details(call: CallbackQuery, state: FSMContext):
    ...
    track_user_activity(user_id, "applicants", "Вопросы ко встрече")
    ...
    await call.message.edit_reply_markup()
    logger.debug(f'Applicant {user_id} entered cha_topics_details handler')
...
async def cha_topics_details2(message: Message, state: FSMContext):
    ...
    await message.answer(text=f"Вопросы для встречи были изменены",
                         reply_markup=applicant_profile_bk,
                              disable_notification=True)
    await state.finish()
    logger.debug(f'Applicant {user_id} entered cha_topics_details2 handler')
